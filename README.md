...
- Fork这个仓库，在仓库 **Settings** -> 左侧列表中的 **Secrets and variables** -> **Actions**，然后在右侧的 **Repository secrets** 中添加如下值：
  ...
  - `PUSH_METHOD`：推送方法，4选1推送方式（pushplus、wxpusher、telegram、sc3）。
  - `PUSHPLUS_TOKEN` or `WXPUSHER_SPT` or `TELEGRAM_BOT_TOKEN`&`TELEGRAM_CHAT_ID` or `SC3_SENDKEY`&`SC3_UID`: 选择推送后填写对应token。
...
| `PUSH_METHOD`              | `pushplus`/`wxpusher`/`telegram`/`sc3`    | **可选**，推送方式，4选1，默认不推送                                       |    secrets     |
...
| `SC3_SENDKEY`             | Server酱³的SendKey                  | 当 `PUSH_METHOD=sc3` 时必填，[获取地址](https://sc3.ft07.com/sendkey) | secrets   |
| `SC3_UID`                 | Server酱³的UID                     | 当 `PUSH_METHOD=sc3` 时选填，用于新版 API，格式为数字用户ID | secrets   |
